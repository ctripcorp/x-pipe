    ...
    static synchronized void schedule(TimerTask task, long delay, long period) {
        if (null == _timer) {
            ...
            ClassLoader ccl = AccessController.doPrivileged(
                    new EvictionTimer.PrivilegedGetTccl());
            try {
                AccessController.doPrivileged(new EvictionTimer.PrivilegedSetTccl(
                        EvictionTimer.class.getClassLoader()));
                _timer = AccessController.doPrivileged(new EvictionTimer.PrivilegedNewEvictionTimer());
            } finally {
                AccessController.doPrivileged(new EvictionTimer.PrivilegedSetTccl(ccl));
            }
        }
        _usageCount++;
