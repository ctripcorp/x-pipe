...
public class RetryableRestOperationsTest {
	...
	public void retryableRestOperationsSuccessTest() throws Exception {
		...
		try {
			...
			assertEquals(true, e.getCause().getCause().getCause() instanceof HttpClientErrorException);
		}
		ctx.close();
	}
	...
	public void retryableRestOperationsFailTest() {
		...
		try {
			...
			assertEquals(true, e.getCause().getCause().getCause() instanceof ResourceAccessException);
		}
