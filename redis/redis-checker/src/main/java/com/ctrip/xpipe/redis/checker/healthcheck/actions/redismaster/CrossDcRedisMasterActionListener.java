...
public class CrossDcRedisMasterActionListener extends OuterClientRedisMasterActionListener implements CrossDcSupport {
    ...
    protected RedisMeta finalMaster(String dcId, String clusterId, String shardId) throws Exception {
        ...
        if (shardMasters.isEmpty())
            throw new OuterClientException(String.format("no active master found in cross dc cluster shard: %s %s", clusterId, shardId));

        if (shardMasters.size() > 1)
            throw new OuterClientException(String.format("too many active masters found in cross dc cluster shard: %s %s", clusterId, shardId));
