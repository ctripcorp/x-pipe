...
public class CrossDcRedisMasterActionListenerTest extends AbstractCheckerTest {

    private CrossDcRedisMasterActionListener listener;
    ...
    public void setupDefaultRedisMasterActionListenerTest() throws Exception {
        listener = new CrossDcRedisMasterActionListener(persistenceCache, metaCache);
        listener = Mockito.spy(listener);
        listener.setOuterClientService(outerClientService);
        instance = newRandomRedisHealthCheckInstance("dc", ClusterType.CROSS_DC, 6379);
    ...
    private XpipeMeta mockXpipeMeta() {
        ...
        DcMeta dcMeta2 = new DcMeta("dc2");
        ClusterMeta clusterMeta2 = new ClusterMeta(clusterId);
        ShardMeta shardMeta2 = new ShardMeta(shardId);
        shardMeta2.addRedis(new RedisMeta().setIp(LOCAL_HOST).setPort(6380).setMaster("0.0.0.0:0"));
        shardMeta2.addRedis(new RedisMeta().setIp(LOCAL_HOST).setPort(6381).setMaster("0.0.0.0:0"));

        xpipeMeta.addDc(dcMeta.addCluster(clusterMeta.addShard(shardMeta)));
        xpipeMeta.addDc(dcMeta2.addCluster(clusterMeta2.addShard(shardMeta2)));

        return xpipeMeta;
    }

    @Test
    public void finalMasterTest() throws Exception {
        ...
        try {
            listener.finalMaster("dc1", "cluster1", "shard3");
        } catch (Exception e) {
