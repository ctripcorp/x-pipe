...
public class TestAbstractMetaStoreTest extends AbstractRedisKeeperTest {
    ...
    public void beforeTestAbstractMetaStoreTest() throws IOException {
        ...
        DefaultMetaStore opened = new DefaultMetaStore(new File("/tmp/xpipe/test"), KEEPER_RUN_ID, asyncFileSystem(), getReplId());
        opened.initialize();
        metaStore = opened;
    }

    @After
    public void afterTestAbstractMetaStoreTest() throws IOException {
        closeMetaStore();
        new File(TMP_META_V2_JSON_FILE).delete();
    }

    private void closeMetaStore() throws IOException {
        if (metaStore != null) {
            metaStore.close();
            metaStore = null;
        }
    ...
    public void testSaveMetaToFile() throws Exception {
        ...
        closeMetaStore();
        DefaultMetaStore reopened = new DefaultMetaStore(new File("/tmp/xpipe/test"), KEEPER_RUN_ID, asyncFileSystem(), getReplId());
        reopened.initialize();
        metaStore = reopened;
