...
public class DefaultIndexStore implements IndexStore {
    ...
    private synchronized GtidSet saveIndex() {
        if (indexWriter != null) {
            try {
                ...
                log.error("[locateGtidRange] failed to save index entry", e);
            }
            return indexWriter.getGtidSet();
        }
        return null;
    ...
    public List<Pair<Long, Long>> locateGtidRange(String uuid, int begGno, int endGno) throws IOException {
        ...
        if(indexReader == null) {
            log.info("[locateGtidRange] index reader is null, uuid: {}, begGno: {}, endGno: {}", uuid, begGno, endGno);
            return result;
        }
        ...
        try {
            ...
            while (changeFileSuccess && !indexReader.noIndex()) {
                try {
                    ...
                    if (null == currentIndexGtidSet || !currentIndexGtidSet.retainAll(reqGtidSet).isEmpty()) {
                        ...
                        for(Pair<Long, Long> range : ranges) {
                            ...
                            if(endBacklogOffset != null) {
                                ...
                                if(endBacklogOffset == null) {
                                    log.warn("[locateGtidRange] cannot determine end offset for file: {}", cmdFileName);
                    ...
                    log.debug("[locateGtidRange] error searching in current index file, trying next, uuid: {}, begGno: {}, endGno: {}", 
                ...
                try {
                    ...
                    log.error("[locateGtidRange] failed to change to next index file", e);
                    changeFileSuccess = false;
                }
            }
            
            if(result.isEmpty()) {
                log.info("[locateGtidRange] GTID not found in range, uuid: {}, begGno: {}, endGno: {}", uuid, begGno, endGno);
            } else {
                log.debug("[locateGtidRange] found {} ranges, uuid: {}, begGno: {}, endGno: {}", 
                        result.size(), uuid, begGno, endGno);
            }
            
            return result;
        } catch (IOException e) {
            log.error("[locateGtidRange] failed to locate GTID range, uuid: {}, begGno: {}, endGno: {}", uuid, begGno, endGno, e);
            return result;
        } finally {
            if(indexReader != null) {
                indexReader.close();
