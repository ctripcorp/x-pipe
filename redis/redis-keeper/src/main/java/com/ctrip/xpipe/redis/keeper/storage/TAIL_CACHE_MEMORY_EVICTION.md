...
- 主体写路径仍调用统一的淘汰策略方法，不实现独立的“快路径”淘汰流程。
- 目标策略中，这种情况不使用水位分档 / 中水位 score，只根据 entry 大小、可安全淘汰范围等信息决定最少淘汰量，并返回不压短的 retention。
- 首版策略可以暂不区分这种情况，忽略相关入参并返回统一默认结果；后续只调整策略方法。
...
- 扫描只要求弱一致性，不构造全局一致快照，也不为扫描期间发生的 entry 创建、关闭、容量变化或 committed 变化重试。每轮开始时读取一次 `globalCommittedBytes`，同时作为本轮水位判断和分档分母；各 entry 的容量在遍历到它时独立读取，因此候选容量与全局分母不要求对应同一时刻。
...
- 第一阶段先完成计量、生命周期、durable 判断、淘汰和调度等主体逻辑，不在主体代码中展开尚未定稿的参数公式。所有水位、大小、score 和时间调整决策集中到一个 `decideEvictionPolicy(...)` 方法，统一返回 `minEvictChunks` 和 `retentionMs`；后续只调整该方法和阈值，不改动主体淘汰流程。
- 首版允许该方法暂时忽略全部入参，固定返回 `minEvictChunks = 1` 和 `retentionMs = expectedMinRetentionMs`；调用方再用 `maxEvictable` 限制最少淘汰量。`minRetainChunks` 的首版默认值也取 `1`。
- 因此首版不区分绝对用量、低 / 中 / 高水位、文件大小或 score，不压短 retention，也不额外放大 floor。相关统计和入参可以先接通，但不要求立即参与决策。
