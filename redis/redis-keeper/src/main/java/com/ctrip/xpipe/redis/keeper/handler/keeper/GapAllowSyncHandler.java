        ...
        if (request.replId.equals("?")) {
            ...
            if (request.offset == -2) {
                ...
                if (curStage.getProto() == ReplStage.ReplProto.PSYNC) {
                    ...
                    XSyncContinue xsyncCont = redisKeeperServer.locateTailOfCmd();
        ...
        if (action.isFull()) {
            ...
            if (action.replStage.getProto() == ReplStage.ReplProto.PSYNC) {
                respStr = String.format("%s %s", PARTIAL_SYNC, action.replId)
                        + (action.protoSwitch || action.keeperPartial ? " "+ action.replOffset : "");
            } else {
                respStr = String.format("XCONTINUE GTID.SET %s GTID.LOST %s MASTER.UUID %s REPLID %s REPLOFF %d",
                        action.gtidSet.toString(), action.gtidLost.toString(), action.replStage.getMasterUuid(), action.replId, action.replOffset);
            }
