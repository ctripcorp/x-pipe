...
insert into REDIS_TBL (id,run_id,dc_cluster_shard_id,redis_ip,redis_port,redis_role,master,redis_master,keepercontainer_id) values(7,'unknown',2,'127.0.0.1',7380,'redis',0,0,null);
insert into REDIS_TBL (id,run_id,dc_cluster_shard_id,redis_ip,redis_port,redis_role,master,redis_master,keepercontainer_id) values(8,'unknown',2,'127.0.0.1',7381,'redis',0,0,null);
...
-- proxy (cross-region 数据通路): jq dc_id=1, oy dc_id=2
-- src_proxy_ids=源 DC 的 TCP(内网) proxy，dst_proxy_ids=目的 DC 的 TLS(公网) proxy
-- 依据 init_data_proxy.sql + AdvancedDcMetaService.getRouteInfo()（依次拼 src/optional/dst 的 proxy uri）
...
insert into proxy_tbl (id,dc_id,uri,monitor_active) values(2, 2,'PROXYTCP://127.0.0.1:11081',1);
insert into proxy_tbl (id,dc_id,uri,monitor_active) values(3, 2,'PROXYTLS://127.0.0.1:11444',1);

-- route (oy <-> jq, 经 proxy 互通)
insert into route_tbl(id, route_org_id, src_dc_id, dst_dc_id, src_proxy_ids, dst_proxy_ids, active, tag, deleted) values (1, 0, 2, 1, '2', '1', 1, 'META', 0);
insert into route_tbl(id, route_org_id, src_dc_id, dst_dc_id, src_proxy_ids, dst_proxy_ids, active, tag, deleted) values (2, 0, 1, 2, '0', '3', 1, 'META', 0);
