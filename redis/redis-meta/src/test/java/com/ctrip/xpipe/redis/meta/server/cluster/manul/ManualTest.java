...
public class ManualTest extends AbstractMetaServerClusterTest {
	...
	private void doSendPing(int serverPort) {
		...
		String address = String.format("http://localhost:%d/%s/%s", serverPort, MetaServerKeeperService.PATH_PREFIX,
				MetaServerKeeperService.PATH_PING);
	...
	public void pingMoving() throws Exception {
		...
		client.setData().forPath(MetaZkConfig.getMetaServerSlotsPath() + "/" + slotId,
				Codec.DEFAULT.encodeAsBytes(info));
