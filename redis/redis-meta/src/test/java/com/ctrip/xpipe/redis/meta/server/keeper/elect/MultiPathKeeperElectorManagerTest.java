...
public class MultiPathKeeperElectorManagerTest extends AbstractKeeperElectorManagerTest {
    ...
    public void beforeDefaultKeeperElectorManagerTest() throws Exception{
        keeper1 = new KeeperMeta().setId("1").setPriority(1);
        keeper2 = new KeeperMeta().setId("2").setPriority(1);
        ...
        when(dcMetaCache.getShardKeepers(anyLong(), anyLong())).thenReturn(Arrays.asList(keeper1, keeper2));

        DefaultKeeperActiveElectAlgorithmManager algorithmManager = new DefaultKeeperActiveElectAlgorithmManager();
        algorithmManager.setDcMetaCache(dcMetaCache);
        algorithmManager.setMetaServerConfig(new UnitTestServerConfig());
        keeperElectorManager.setKeeperActiveElectAlgorithmManager(algorithmManager);
    ...
    public void testLeaderSelect() throws Exception {
        ...
        Assert.assertEquals(Arrays.asList(MetaCloneFacade.INSTANCE.clone(keeper1), MetaCloneFacade.INSTANCE.clone(keeper2)), surviveKeepers.get());
    ...
    public void testPathSwitch() throws Exception {
        ...
        Assert.assertEquals(Arrays.asList(MetaCloneFacade.INSTANCE.clone(keeper2)), surviveKeepers.get());
        ...
        Assert.assertEquals(Arrays.asList(MetaCloneFacade.INSTANCE.clone(keeper2), MetaCloneFacade.INSTANCE.clone(keeper1)), surviveKeepers.get());
        Assert.assertEquals("2", activeKeeper.get().getId());
