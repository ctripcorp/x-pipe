...
public class DefaultSentinelManager implements SentinelManager{
	...
	public void addSentinel(String clusterId, String shardId, RedisMeta redisMaster, ExecutionLog executionLog) {
		...
		for(int i=0; i < addSize; i++){
			...
			SentinelAdd command = new SentinelAdd(clientPool, sentinelMonitorName, redisMaster.getIp(), redisMaster.getPort(), quorum);
			try {
	...
	public void removeSentinel(String clusterId, String shardId, ExecutionLog executionLog) {
		...
		for(Sentinel sentinel : realSentinels){
			...
			SentinelRemove sentinelRemove = new SentinelRemove(clientPool, sentinelMonitorName);
			try {
	...
	private List<Sentinel> getRealSentinels(List<InetSocketAddress> sentinels, String sentinelMonitorName, ExecutionLog executionLog) {
		...
		for(InetSocketAddress sentinelAddress: sentinels){
			...
			SimpleObjectPool<NettyClient> clientPool = keyedClientPool.getKeyPool(sentinelAddress);
			Sentinels sentinelsCommand = new Sentinels(clientPool, sentinelMonitorName);
			try {
				...
				if(realSentinels.size() > 0){
					realSentinels.add(new Sentinel(sentinelAddress.toString(), sentinelAddress.getHostName(), sentinelAddress.getPort()));
					break;
				}
			} catch (InterruptedException | ExecutionException e) {
