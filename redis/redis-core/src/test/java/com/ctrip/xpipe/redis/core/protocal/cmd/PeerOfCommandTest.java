...
public class PeerOfCommandTest extends AbstractRedisTest {
    ...
    public void testPeerOfWithProxy() throws Exception {
        ...
        PeerOfCommand command = new PeerOfCommand(getXpipeNettyClientKeyedObjectPool().getKeyPool(new DefaultEndPoint("localhost", port)),
                1,
                new DefaultEndPoint("127.0.0.1", 0, new DefaultProxyConnectProtocolParser().read("PROXY ROUTE PROXYTCP://127.0.0.1:1")),
                scheduled);
        ...
        command = new PeerOfCommand(getXpipeNettyClientKeyedObjectPool().getKeyPool(new DefaultEndPoint("localhost", port)),
                1,
                new DefaultEndPoint("127.0.0.1", 0, new DefaultProxyConnectProtocolParser().read("PROXY ROUTE PROXYTCP://127.0.0.1:1 PROXYTLS://127.0.0.1:2")),
                scheduled);
        ...
        command = new PeerOfCommand(getXpipeNettyClientKeyedObjectPool().getKeyPool(new DefaultEndPoint("localhost", port)),
                1,
                new DefaultEndPoint("127.0.0.1", 0, new DefaultProxyConnectProtocolParser().read("PROXY ROUTE PROXYTCP://127.0.0.1:1 PROXYTLS://127.0.0.1:2 PROXYTLS://127.0.0.1:3")),
                scheduled);
