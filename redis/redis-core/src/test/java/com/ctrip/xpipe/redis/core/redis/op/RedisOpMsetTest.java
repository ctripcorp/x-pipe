...
public class RedisOpMsetTest extends AbstractRedisOpParserTest {
    ...
    public void testDivideGtidMsetTest() {
        RedisOp redisOp = parser.parse(Arrays.asList("gtid", "a1:100", "0", "mset", "k1", "v1", "k2", "v2", "k3", "v3", "k4", "v4", "k5", "v5").toArray());
        Assert.assertEquals(RedisOpType.MSET, redisOp.getOpType());
        ...
        Assert.assertEquals(RedisOpType.MSET, subOp1.getOpType());
        Assert.assertEquals(RedisOpType.MSET, subOp2.getOpType());
        Assert.assertArrayEquals(strList2bytesArray(Arrays.asList("gtid", "a1:100", "0", "mset", "k1", "v1", "k3", "v3", "k5", "v5")), subOp1.buildRawOpArgs());
        Assert.assertArrayEquals(strList2bytesArray(Arrays.asList("gtid", "a1:100", "0", "mset", "k2", "v2", "k4", "v4")), subOp2.buildRawOpArgs());
    }
