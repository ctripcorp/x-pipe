...
public class AbstractBulkStringParserTest extends AbstractRedisProtocolTest{

    private AbstractBulkStringParser bs = new CommandBulkStringParaser(new AbstractBulkStringParserTest.TestPayload());
    ...
    public void testEOF(){
        ...
        String eof = randomString(AbstractBulkStringEoFJudger.BulkStringEofMarkJudger.MARK_LENGTH);
        String buff = "$EOF:" + eof + "\r\n" + content + eof;

        for(int i=1; i <= eof.length();i++){

            bs = new CommandBulkStringParaser(new AbstractBulkStringParserTest.TestPayload());
    ...
    public void testEOFSplit(){
        bs = new RdbBulkStringParser(new AbstractBulkStringParserTest.TestPayload());
        String eof = randomString(AbstractBulkStringEoFJudger.BulkStringEofMarkJudger.MARK_LENGTH);
        String buff = "$EOF:" + eof + "\r\n" + content + eof;
    ...
    public void testCommandEndCRError() {
        ...
        Assert.assertEquals(e.getMessage(), String.format("Parse the Redis command protocol Error: Here should be '\r' ,but it's %s", (char)'\n'));
        Assert.assertEquals(result, null);
    }
