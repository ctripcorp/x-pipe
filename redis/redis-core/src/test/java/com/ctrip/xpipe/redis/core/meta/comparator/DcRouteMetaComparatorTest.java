...
public class DcRouteMetaComparatorTest extends AbstractComparatorTest {
    ...
    public void testCompareWithRouteAdd() {
        future.addRoute(new RouteMeta(1000L).setRouteInfo("PROXYTCP://127.0.0.1:80 PROXYTLS://127.0.0.7:443").setSrcDc("fra").setDstDc("jq"));
        comparator = new DcRouteMetaComparator(current, future);
        comparator.compare();
        ...
        future.addRoute(new RouteMeta(2000L).setTag(Route.TAG_META));
        comparator = new DcRouteMetaComparator(current, future);
        comparator.compare();
