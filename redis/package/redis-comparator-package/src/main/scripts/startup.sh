...
function getPortFromPath(){
    path=$1
    # Last `_` + exactly 3-5 digits (not a prefix of a longer number).
    # Proxy/keeper take the first 3-5 digits in the path, which turns
    # app.id 100077310 in /opt/app/{appId}_{port} into 10007.
    port=""
    remaining=$path
    while [[ "$remaining" == *_* ]]; do
        remaining="${remaining#*_}"
        if [[ "$remaining" =~ ^([0-9]{3,5})([^0-9]|$) ]]; then
            port="${BASH_REMATCH[1]}"
        fi
    done
    echo $port
...
function getSafeXmx() {
    # Heap: 50% of machine memory, cap 8G (on-heap RingBuffer, spec Q17 / §4.10)
    total=`getTotalMem`
    SAFE_PERCENT=50
    MAX_MEM=8
...
function tryRemoveJarLog() {
    logdir=$1
    appname=$2
    find "$logdir" -type f -name "${appname}_*.log" -delete
}
