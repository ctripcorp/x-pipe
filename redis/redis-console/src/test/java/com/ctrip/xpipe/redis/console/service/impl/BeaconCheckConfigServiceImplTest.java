...
public class BeaconCheckConfigServiceImplTest {
    ...
    private DcClusterShardTbl dcClusterShardTbl;

    @Before
    public void setup() {
        dcClusterShardTbl = new DcClusterShardTbl();
        dcClusterShardTbl.setDcClusterShardId(1L);
        ...
        Mockito.when(dcClusterShardService.find("jq", "cluster1", "shard1")).thenReturn(dcClusterShardTbl);
    ...
    public void testStopBeaconCheck() throws Exception {
        ...
        Mockito.verify(dcClusterShardService).updateMetaExcludeUntilTimestamp(Mockito.argThat(shard ->
                shard.getMetaExcludeUntilTimestamp() != null
                        && shard.getMetaExcludeUntilTimestamp() > System.currentTimeMillis()));
    }
