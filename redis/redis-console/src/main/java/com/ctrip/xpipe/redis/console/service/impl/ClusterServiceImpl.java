...
public class ClusterServiceImpl extends AbstractConsoleService<ClusterTblDao> implements ClusterService {
	...
	public List<UnmatchedClusterRouteInfoModel> findUnmatchedClusterRoutes() {
		...
		for(DcMeta dcMeta : xpipeMeta.getDcs().values()) {
			...
			for (ClusterMeta clusterMeta : dcMeta.getClusters().values()) {
				result.addAll(findUnmatchedClusterRoutes(clusterMeta,dcMeta.getId(), allDcRoutes));
			}
		}
	...
	private List<UnmatchedClusterRouteInfoModel> findUnmatchedClusterRoutes(ClusterMeta clusterMeta, String dcName, Map<String, List<RouteMeta>> allDcRoutes) {
		...
		OrganizationTbl organizationTbl= organizationService.getOrganization(clusterMeta.getOrgId());
