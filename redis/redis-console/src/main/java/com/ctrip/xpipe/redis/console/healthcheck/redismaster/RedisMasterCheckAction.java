 ...
 * Oct 08, 2018
 */
public class RedisMasterCheckAction extends AbstractDCLAHealthCheckAction {
    ...
    public RedisMasterCheckAction(ScheduledExecutorService scheduled, RedisHealthCheckInstance instance,
                                  ExecutorService executors, MetaCache metaCache, RedisService service) {
    ...
    protected void doScheduledTask() {
        ...
        if(metaCache.inBackupDc(info.getHostPort())) {
            logger.debug("[doScheduledTask] not in backup dc: {}", info);
            return;
        }
