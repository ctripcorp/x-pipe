...
public class ClusterDao extends AbstractXpipeConsoleDAO{
	...
	public ClusterTbl createCluster(final ClusterTbl cluster) throws DalException {
	    ...
	    if (!ClusterType.lookup(newCluster.getClusterType()).supportMultiActiveDC()) {
			...
			protoDcCluster.setDcId(activeDc.getId()).setClusterId(newCluster.getId()).setActiveRedisCheckRules("");
	...
	public int bindDc(final ClusterTbl cluster, final DcTbl dc, SentinelGroupModel sentinel) throws DalException {
		...
		if(null == existingDcCluster) {
			...
			proto.setDcId(dc.getId()).setClusterId(cluster.getId()).setActiveRedisCheckRules("");
